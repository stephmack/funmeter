...
// along with this program.  If not, see <http://www.gnu.org/licenses/>. test
...
func (scm SCM) String() string {
	t := time.Now()
	db, err := sql.Open("mysql","root:raspberry@tcp(127.0.0.1:3306)/Utils")
	if err != nil {
		fmt.Println(err)
	}
	stmt, err := db.Prepare("INSERT INTO Utils.scm(DT,ID,Type,Tamp1,Tamp2,Reading) VALUES(?,?,?,?,?,?)")
	if err != nil {
		fmt.Println(err)
	}
	res, err := stmt.Exec(t.Format("2006-01-02 15:04:05.000"),scm.ID, scm.Type, scm.TamperPhy, scm.TamperEnc, scm.Consumption)
	fmt.Println(res)
	defer db.Close()
	
//	var (
//	        basePath = "../data"
//                indexFile = filepath.Join(basePath, "csm.csv")
//        )
//	file, err := os.OpenFile(indexFile, os.O_RDWR|os.O_APPEND, 0777)
//	if err != nil {
//      		fmt.Println(err)
//	}
//	fmt.Fprintf(file,"%8d,%2d,%02X,%02X,%8d\n",
//                scm.ID, scm.Type, scm.TamperPhy, scm.TamperEnc, scm.Consumption,
//	)	
//	file.Close()
	...
	//return fmt.Sprintf("%8d,%2d,%02X,%02X,%8d,0x%04X",
        //        scm.ID, scm.Type, scm.TamperPhy, scm.TamperEnc, scm.Consumption, scm.ChecksumVal,
	)
}
